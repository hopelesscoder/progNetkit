...
/etc/init.d/ulogd stop >/dev/null
	...
	#iptables --table nat -A POSTROUTING --jump MASQUERADE --out-interface eth0 --source 192.0.0.0/24 #--destination 0/0
	#iptables --table nat -A POSTROUTING --jump MASQUERADE --out-interface eth0 --source 150.0.0.0/24 #--destination 0/0
	iptables --table nat --append PREROUTING --in-interface eth1 --destination 120.0.0.10  --jump DNAT --to-destination  10.0.0.2
	iptables --table nat --append PREROUTING --in-interface eth1 --destination 120.0.0.15  --jump DNAT --to-destination  172.0.0.6




 
	#iptables --table nat -A POSTROUTING --out-interface eth0 --jump SNAT --to 10.0.0.2
